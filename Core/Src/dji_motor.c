...
volatile uint32_t g_dji_feedback_id_count[9] = {0U};
volatile uint32_t g_dji_rejected_feedback_count = 0U;
volatile uint32_t g_dji_unmatched_feedback_count = 0U;
volatile uint32_t g_dji_unmatched_feedback_id = 0U;
...
static HAL_StatusTypeDef DjiMotor_SendCurrentFrame(uint32_t std_id, const uint8_t data[8])
{
    if (dji_hcan == 0) {
        return HAL_ERROR;
    }
...
void DjiMotor_OnCanFeedback(uint32_t std_id, const uint8_t data[8])
{
    if (data == 0 || std_id < DJI_MOTOR_CAN_ID_MIN || std_id > DJI_MOTOR_CAN_ID_MAX) {
        g_dji_rejected_feedback_count++;
        return;
    }

    g_dji_feedback_id_count[std_id - 0x200U]++;

    int index = DjiMotor_FindByCanId(std_id);
    if (index < 0) {
        g_dji_unmatched_feedback_count++;
        g_dji_unmatched_feedback_id = std_id;
        return;
    }
